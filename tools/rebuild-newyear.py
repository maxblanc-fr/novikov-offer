...
SERVICES_NOTE = """
  Раздел «Дополнительные услуги» остался по ценам взрослой страницы.
  Новогодние цены на доп. активности заказчик присылает отдельно — внесите их
  сюда отдельным словарём, когда они появятся, чтобы пересборка их не теряла.
"""
