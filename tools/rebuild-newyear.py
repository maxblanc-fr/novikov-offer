...
SERVICE_CARDS = {
    ...
    # Сомелье здесь снова оплачивается за гостя (2 600 ₽), как во взрослой, —
    # фикс 60 000 ₽ за мероприятие заказчик отменил 11.09.2026. Поэтому меняются
    # только цифры, а формула расчёта остаётся исходной.
    "Дегустационные казино": [
        ("Цена = сомелье 2 200 ₽/гость", "Цена = сомелье 2 600 ₽/гость"),
        ("Price = sommelier 2 200 ₽/guest", "Price = sommelier 2 600 ₽/guest"),
        ('p:"1 050 ₽/гость · раунд", n:"3 раунда ≈ 64 200 ₽"',
         'p:"1 300 ₽/гость · раунд", n:"3 раунда ≈ 78 000 ₽"'),
        ('p:"1 200 ₽/гость · раунд", n:"3 раунда ≈ 69 600 ₽"',
         'p:"1 500 ₽/гость · раунд", n:"3 раунда ≈ 85 200 ₽"'),
        ("плюс работа сомелье 2 200 ₽/гость", "плюс работа сомелье 2 600 ₽/гость"),
        ("plus the sommelier at 2 200 ₽/guest", "plus the sommelier at 2 600 ₽/guest"),
        ('price:{ru:"Сомелье 2 200 ₽/гость · 3 раунда", en:"Sommelier 2 200 ₽/guest · 3 rounds"}',
         'price:{ru:"Сомелье 2 600 ₽/гость · 3 раунда", en:"Sommelier 2 600 ₽/guest · 3 rounds"}'),
    ],
    "Коктейльный мастер-класс": [
        ('p:"2 200 ₽/гость", n:"≈ 26 400 ₽"', 'p:"2 700 ₽/гость", n:"≈ 32 400 ₽"'),
        ('p:"4 400 ₽/гость", n:"≈ 52 800 ₽"', 'p:"5 400 ₽/гость", n:"≈ 64 800 ₽"'),
        ('p:"6 600 ₽/гость", n:"≈ 79 200 ₽"', 'p:"8 100 ₽/гость", n:"≈ 97 200 ₽"'),
        ("<b>2 200 ₽ за коктейль на гостя</b>", "<b>2 700 ₽ за коктейль на гостя</b>"),
        ("<b>2 200 ₽ per cocktail per guest</b>", "<b>2 700 ₽ per cocktail per guest</b>"),
        ('price:{ru:"2 200 ₽ / гость за коктейль", en:"2 200 ₽ / guest per cocktail"}',
         'price:{ru:"2 700 ₽ / гость за коктейль", en:"2 700 ₽ / guest per cocktail"}'),
    ],
    # Рисование — одна цена на акрил, вино и кофе; отдельной ставки за технику нет.
    "Арт": [
        ('price:{ru:"Рисование 6 000 ₽ · свечеварение 5 500 ₽ / гость", en:"Painting 6 000 ₽ · candle making 5 500 ₽ / guest"}',
         'price:{ru:"Рисование 7 200 ₽ · свечеварение 6 600 ₽ / гость", en:"Painting 7 200 ₽ · candle making 6 600 ₽ / guest"}'),
    ],
}
...
SERVICE_TMAP = {
    ...
    '"3 раунда ≈ 64 200 ₽":"3 rounds ≈ 64 200 ₽"':     '"3 раунда ≈ 78 000 ₽":"3 rounds ≈ 78 000 ₽"',
    '"3 раунда ≈ 69 600 ₽":"3 rounds ≈ 69 600 ₽"':     '"3 раунда ≈ 85 200 ₽":"3 rounds ≈ 85 200 ₽"',
    '"2 200 ₽/гость":"2 200 ₽/guest"': '"2 700 ₽/гость":"2 700 ₽/guest"',
    '"4 400 ₽/гость":"4 400 ₽/guest"': '"5 400 ₽/гость":"5 400 ₽/guest"',
    '"6 600 ₽/гость":"6 600 ₽/guest"': '"8 100 ₽/гость":"8 100 ₽/guest"',
}

# Те же цены продублированы в шапках модалок и в подписях арт-галереи — это
# render-функции, они лежат вне объекта DATA, поэтому правятся отдельно.
OUTSIDE_DATA = {
    '${LANG==="ru"?"2 200 ₽ / гость за коктейль":"2 200 ₽ / guest per cocktail"}':
    '${LANG==="ru"?"2 700 ₽ / гость за коктейль":"2 700 ₽ / guest per cocktail"}',
    'painting:{ru:"6 000 ₽ / гость", en:"6 000 ₽ / guest"}':
    'painting:{ru:"7 200 ₽ / гость", en:"7 200 ₽ / guest"}',
    'candles:{ru:"5 500 ₽ / гость", en:"5 500 ₽ / guest"}':
    'candles:{ru:"6 600 ₽ / гость", en:"6 600 ₽ / guest"}',
}
...
def apply_service_cards(lines):
    ...
    whole = "\n".join(lines)
    for old, new in OUTSIDE_DATA.items():
        if old not in whole:
            sys.exit(f"вне DATA не найдено — {old[:70]}")
        whole = whole.replace(old, new)
    lines[:] = whole.split("\n")
    print(f"  модалки и галерея: {len(OUTSIDE_DATA)} правок")
...
def apply_prices(lines, mapping, regions, label):
    ...
    # Граница обязательна: без неё маска «6 600 ₽» из free-flow срезала бы
    # начало строки «6 600 ₽/гость» коктейльного мастер-класса в словаре
    # переводов. Длинные ключи идут первыми, поэтому «от 12 500 ₽/чел»
    # выигрывает у «12 500 ₽», а не наоборот.
